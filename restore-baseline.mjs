...
patchFile('src/App.jsx', [
  ...
  [
    ...
    "const baselineFemale=false, isHDCharacter=character==='male-hd'||character==='female-hd', isProxyCharacter=character==='male-proxy', rigProfile=status?.profile||null",
  ...
  [
    ...
    "여성 강사는 기존 승인 턴어라운드에서 복구한 리깅 HD GLB를 사용합니다. <b>리그 테스트</b>는 캘리브레이션 GLB를 사용합니다.",
    'female help text'
  ],
  [
    ...
    "<div className={'status '+(baselineFemale||status.loaded?'ok':'warn')}>{baselineFemale?'여성 HD GLB 로드 대기':status.loaded?'GLB 로드됨 · 휴머노이드 본 매핑 '+(status.mapped?'완료':'부분'):'GLB 미탑재 시 절차형 Rig로 자동 테스트'}</div>",
    'female status'
  ],
  [
...
console.log('Female HD GLB production path restored. Model export enabled.')
